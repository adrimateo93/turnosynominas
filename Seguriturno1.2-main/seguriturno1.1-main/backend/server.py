...
async def get_companies(user=Depends(get_current_user)):
    ...
    # Si no hay empresas, crear las dos por defecto
    if len(companies) == 0:
        default_companies = [
            {
                "id": str(uuid.uuid4()),
...
async def calculate_payroll(year: int, month: int, company_id: Optional[int] = None, user=Depends(get_current_user)):
    ...
    overtime_hours = sum(s.get("overtime_hours", 0) for s in shifts if s.get("shift_type") == "normal")
    ...
    horas_computadas = total_hours_worked
    ...
    # Pluses que cotizan (incluyendo plus servicio)
    pluses_cotizables = plus_nocturnidad + plus_festivo + paga_extra_mes + plus_servicio_importe
    
    # Pluses que NO cotizan (transporte y vestuario tienen límites exentos)
    pluses_no_cotizables = plus_transporte + plus_vestuario
